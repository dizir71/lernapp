Ersetze den Konflikt komplett durch diese bereinigte app.js:
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadJson(url) {
    try {
      const res = await fetch(`${url}?${CACHE_BUSTER()}`, { cache: "reload" });
  ...
  async function buildQuestionPool() {
    ...
    // Dubletten nach question_text bereinigen
    const seen = new Set();
    const unique = [];
  ...
  function triggerFireworks() {
    ...
    const interval = setInterval(() => {
      ...
      const particleCount = 50 * (t / duration);
  ...
  function saveAndDisplayExamResults(score, total) {
    ...
    if (percentage >= 91) { grade = 'Note 1 (Sehr Gut)'; message = 'Hervorragend! Du bist ein echter Experte!'; triggerFireworks(); }
    else if (percentage >= 81) { grade = 'Note 2 (Gut)'; message = 'Starke Leistung! Du bist auf dem besten Weg.'; }
    else if (percentage >= 67) { grade = 'Note 3 (Befriedigend)'; message = 'Gut gemacht, das sitzt schon ganz gut.'; }
    else if (percentage >= 50) { grade = 'Note 4 (Genügend)'; message = 'Knapp, aber bestanden. Wiederhole die unsicheren Themen.'; }
    else { grade = 'Note 5 (Nicht Genügend)'; message = 'Das war leider nichts. Nutze den Lernmodus und versuch es erneut!'; }
  ...
  function saveAnswerAndProceed() {
    const q = currentQuestions[currentQuestionIndex];
    const selectedAnswers = new Set();
    UI.answers.querySelectorAll('.opt.selected').forEach(btn => selectedAnswers.add(btn.dataset.answer));
  ...
  // Test-Mode Rendering überschreiben
  ...
  renderQuestion = function () {
    if (mode === 'test') {
      ...
      const questionType = q.type || 'single_choice';
      const options = q.possible_answers || (questionType === 'true_false' ? ['true', 'false'] : []);
      options.forEach(opt => { answerHtml += `<button class="opt" data-answer="${opt}">${opt}</button>`; });
      ...
      UI.answers = document.getElementById("answers");
      UI.answers.querySelectorAll('.opt').forEach(btn => btn.addEventListener('click', () => handleAnswerInTestMode(btn, q)));
  ...
  UI.chkExternal.onchange = () => { console.log("Checkbox changed, reloading questions..."); startApp(); };

  // Initial boot
  startApp();
});

Konfliktmarker entfernen. Datei speichern. In GitHub “Mark as resolved”.